...
Authors:           Kaylee Grafton, Frederika Delcuore, Samuel Larocca, Fatema Yasmin
...
def drawPiece(row, col, cell_size):
    '''
    Function drawPiece
    Written by:             Kaylee Grafton
    Description:            draws pieces where the player clicked
    '''
    # checks to see if the space is already taken
    if board[row][col] == ' ':
        # determines x and y based on the row and col the player clicked on
        x = -cell_size + (col * cell_size)
        y = cell_size - ((row * cell_size) + 30) #offsets y slightly to make it centered
        turtle.hideturtle()
        turtle.penup()
        turtle.goto(x, y)
        turtle.pendown()
